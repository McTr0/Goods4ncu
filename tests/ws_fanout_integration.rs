...
async fn fanout_fails_fast_in_replicated_mode_when_redis_unreachable() {
    ...
    assert!(
        ...
        "replicated runtime must clear publisher on failed start"
    );
    assert!(
...
async fn two_instances_deliver_across_processes() {
    ...
    tokio::time::timeout(Duration::from_secs(60), test_body)
        .await
        .expect("two_instances_deliver_across_processes must complete within 60s outer timeout");
}
