...
class _RecordingUserService extends UserService {
  ...
  Future<http.Response> get(Uri url, Map<String, String> headers) async {
    lastUri = url;
    if (url.path == '/api/persona/catalog') {
      return http.Response(
        '{"style_version":"v1","representation_modes":["trait_mapped","role_character"],"appearance":{"palette":["teal"]},"self_descriptions":[],"contact_postures":["leave_message"]}',
        200,
      );
    }
...
void main() {
  ...
  group('persona catalog', () {
    test('reads only the server-owned role and skin choices', () async {
      final service = _RecordingUserService();

      final catalog = await service.getSocialPersonaCatalog();

      expect(catalog.styleVersion, 'v1');
      expect(catalog.representationModes, ['trait_mapped', 'role_character']);
      expect(catalog.appearance['palette'], ['teal']);
      expect(service.lastUri?.path, '/api/persona/catalog');
    });
  });
}
