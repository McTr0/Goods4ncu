...
- 用户能区分自己的角色化呈现、平台 Agent（候选工作名“小樟”）和真实对方；Agent 不因空闲、Push 或停留主动出现，连接期间不把角色动画当作在线/已读/正在输入。
