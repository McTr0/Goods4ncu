...
| 事实来源 | 本仓库 Rust/Flutter 测试、`scripts/` 下可执行演练（含真实 MinIO/Redis/Postgres）、迁移 0001–0065 |
...
| 空库/升级库迁移均验证（含真实升级库上的 legacy 值归一化） | CI migration job + 0040/0041 升级路径实测 + 2026-08-12 全量空库迁移至 `0065`；file/link 对象生命周期仍需接入真实对象存储回调 |
