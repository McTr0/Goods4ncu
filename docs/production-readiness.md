...
| 事实来源 | 本仓库 Rust/Flutter 测试、`scripts/` 下可执行演练（含真实 MinIO/Redis/Postgres）、迁移 0001–0069 |
...
| 空库/升级库迁移均验证（含真实升级库上的 legacy 值归一化） | CI migration job + 0040/0041 升级路径实测 + 全量空库迁移至 `0069`；`0068` 清理 attention 兼容影子列，`0069` 为媒体审核 processing 任务增加可回收 lease；2026-08-12 在 `0069` 后重跑 production rehearsal，真实 MinIO OSS probe、signed DELETE、撤销审计和远端对象清理均通过 |
