...
| 当前状态 | 已有认证、封禁、文本规则、异步图片审核、举报、屏蔽和管理员审计；统一审核案件、申诉、租户策略与密钥管理属于目标态 |
...
[目标态] 生产管理员和校园运营必须使用 MFA；敏感操作需要近期认证。账号安全事件应能撤销全部会话，并向用户发送不包含敏感细节的通知。
...
[已实现] 图片 URL 可以进入 `moderation_jobs` 异步处理。

[目标态] 上传成功不等于公开成功。对象先进入私有隔离区，审核通过后才生成可公开 CDN URL。审核期间使用安全占位图，不直接展示原始对象。
...
[目标态] 每个机器拒绝、人工处理、举报升级和申诉形成统一案件。普通用户看到案件状态、原因类别和下一步，不看到审核人员身份、举报人或内部规则。

申诉必须由未参与原决定的审核者复核高风险案件。改判会恢复可见性、写审计并用于评估误伤率，但不会自动训练模型。
...
1. 停止向新用户宣传和默认展示 Secret Chat。
2. 禁止创建新的长期 E2EE session，保留已有历史的只读兼容窗口。
