...
[已实现] 对象存储层面：bucket 私有（无匿名读），商品、头像和共享对象在私有部署使用只绑定服务器 key 的 presigned PUT，服务端只对 approved 媒体下发短期 presigned GET，因此已知直链在存储层同样不可访问。SocialPersona 角色/皮肤不接受用户上传，直接使用系统目录 token。对真实 S3 实现验证过匿名 403、篡改签名拒绝、过期拒绝和 presigned PUT。[目标态] CDN 前置与缩略图派生仍待接入。
...
- 用户可以随时恢复普通头像、隐藏分身或改选目录项；改选会再次要求显式发布。目录服务故障不影响资料、留言和连接。
