...
[已实现] 对象存储层面：bucket 私有（无匿名读），商品、legacy 头像和共享对象在私有部署使用只绑定服务器 key 的 presigned PUT，服务端只对 approved 媒体下发短期 presigned GET，因此已知直链在存储层同样不可访问。SocialPersona Avatar 角色/皮肤不接受用户上传，直接使用系统目录 token；legacy 头像媒体只为旧客户端兼容保留，新客户端不渲染或上传。对真实 S3 实现验证过匿名 403、篡改签名拒绝、过期拒绝和 presigned PUT。[目标态] CDN 前置与缩略图派生仍待接入。
...
[部分实现] 角色分身是可编辑的公开展示资源，不是身份证明、真人照片认证或可自主行动的代理。校园认证徽标只由 `CampusMembership` 决定，不能因角色外观、服装、标签或模型判断改变。`0063_social_personas` 存受控 token 和用户选择标签；`0080_system_persona_catalog_only` 将角色/皮肤收敛为服务端版本化目录，用户不能上传、导入图片、提供外部 URL 或提交开放式人格指令。`0070_social_persona_assets`/`0071_social_persona_asset_upload_expiry` 仅保留历史回滚和远端清理字段，既有用户素材迁移时撤销且不再公开投影。

角色呈现模式只保留特征映射和完全角色化，后者标注“角色形象”；角色与皮肤均来自系统目录，不声称与真人外貌一一对应。平台不得从角色推断性别、年龄、族裔、健康、性取向或其他敏感属性，也不得把这些推断用于联系权限或推荐。

生成与发布遵循以下边界：

- 默认使用统一风格和系统审核过的部件，目录由平台发布版本；不接受开放式 prompt、照片转换、第三方皮肤包或自定义素材，防止批量同质化、色情化和冒用知名人物或其他用户。
- 用户可以随时隐藏分身或改选目录项；改选会再次要求显式发布。未配置、归档或目录服务故障时使用默认系统 Avatar，不恢复照片、`avatar_url` 或用户上传头像；目录故障不影响资料、留言和连接。
...
角色编辑器还要支持减少动态、静态替代文本、高对比度和同角色静态 poster fallback。Flutter 采用类型化本地状态机 + 版本化 Sprite Atlas，按 24px 静态关键帧、48px 低频动效、160px 完整交互分级，加载失败、reduced-motion 或低能力设备均回退同角色 poster。动作 cue 不接受自由文本或消息情绪推断，不能表达 online、read、typing、连接接受或成交确认。身份与接近方式不能只靠颜色、姿态或动画表达。产品不引入好感度、喂养、连续签到或随机召回，因为这些机制会把身份表达变成注意力操控。
