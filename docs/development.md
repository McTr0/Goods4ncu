...
新增迁移文件放在 `migrations/`，文件名使用递增编号，例如 `0018_add_example.sql`。迁移要只向前执行，不要修改已经合并过的旧迁移；唯一例外是已合并的兼容迁移引用了更早阶段已经删除的旧表、导致全新数据库无法启动时，可以把旧操作改成“表存在才执行”的幂等保护，但必须同时验证空库和升级库，并在提交说明中记录原因。
