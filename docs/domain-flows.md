...
公开用户主页展示必要身份、SocialPersona Avatar、在出/在收内容和用户主动公开的收款码。Avatar 点击进入个人公开主页，个人设置页不再用重复菜单卡片代替这一入口；未配置时显示默认系统 Avatar，不显示照片或 `avatar_url` 回退。
...
角色创建遵循“系统目录选择、用户确认、发布后公开”：

```text
读取当前 style_version 的系统角色/皮肤目录
  -> 用户选择目录项、自我描述和公开接近方式
  -> 服务端再次校验 token 与可访问性
  -> 用户预览角色卡并主动发布
```

呈现模式只保留特征映射和完全角色化；完全角色化必须显示“角色形象”，不得暗示真人外貌认证。角色和皮肤只能来自系统目录，不能上传、导入或提交外部 URL。校园 membership 徽标来自认证事实，与角色风格、服装或用户自述分开显示。

SocialPersona Avatar 是唯一的用户头像呈现。当前 Flutter 使用严格类型化的本地动作 cue 与可替换 renderer：默认系统角色采用版本化 Sprite Atlas，已配置角色继续使用受控 token 的代码绘制；24px 静态，48/160px 允许无语义的低频本地 idle。尺寸为 24px 静态关键帧、48px 低频动效、160px 完整交互，加载失败、reduced-motion 或不支持动效时回退同一角色的静态 poster。动作不能表达 online、read、typing、连接接受或成交确认，也不能由聊天内容自动推断用户角色情绪。

公开接近方式只能来自用户选择或用户预先定义的规则，包括可留言、可请求连接、忙和稍后；它不是 online presence。系统不得根据页面打开、键盘、日历、地理位置或消息历史偷偷修改。修改只影响未来展示和连接门禁，不回写历史会话，也不替用户发送说明。

AI 只能生成资料草稿或单次回复草稿；不能生成、导入或自动发布角色皮肤，不能自动代聊或学习用户语气去维持关系。私有核心 Avatar 小昌与对外 SocialPersona 必须使用不同标识和上下文提示；小昌从中央主导航进入，不作为联系人或对话参与者。
...
### 目标态：在 realtime Conversation 内投影 Relationship Space

同一校园内的一对用户只有一个 Thread 长期索引；点击联系人后进入独占整页，再由每一段 realtime Conversation 承载自己的共同空间。mail 和已结束历史保持独立卡片，不与共同空间混排：

```text
打开联系人 Thread 全页
  -> 展开一段 realtime Conversation
  -> 共同空间占据除输入与操作区之外的全部主画布
  -> 对方 SocialPersona Avatar 固定左上，我方固定右下
  -> 当前 Conversation 的消息、连接状态与共享信息在共同空间内部呈现
  -> 允许“拍一拍”等显式本地动作；没有服务端回执前不声称对方收到或回应
  -> 历史 Thread、其他 Conversation 与跨会话时间轨迹不进入当前共同空间
  -> 用户选择留下留言，或请求一次新的连接
  -> 文件、链接、商品或地点成为可回到的共享对象
```

实时聊天页不再加载或显示独立 `AgreementCard / HandoffPrompt`。“说好的事”曾在 2026-08-14 的 `e4faa6d` 接入，但它把第二套结构化流程堆在共同空间下方，与 realtime 以空间为主画布的职责冲突，已于 2026-08-15 移除。服务端历史数据暂不做破坏性迁移。
...
角色随纯本地布局状态缩放是允许的，例如密集阅读时缩成角落 Avatar；角色不得根据对方输入、打开页面、通知预览或后台活动变化。连接期间缩细 Memory Rail、弱化角色，表达“当前关注人”；连接结束后恢复历史入口。
...
Mail 创建后进入 open，主题 1–120 字、正文 1–2000 字，无需接受即可发送。发件人可以选择 `ordinary`（有空时看）或 `today`（希望今天处理）；这只是礼貌的时间尺度声明，不改变接收方通知优先级或忙碌/静音规则。发送成功只表示消息已持久化到服务器；没有设备 ACK 时不宣称“已送达”。Mail 不向发件人展示 typing 或对方 read 状态，接收端可以本地记录 `LOCALLY_SEEN`，也可以主动发送 `received`、`will_review` 或 `completed`。
...
[目标态] 群组不直接套用一对一连接。长期 `Group` 保存成员与群留言，临时 `Discussion` 承载一次有开始、参与者和结束的实时讨论；未加入讨论的成员不被标记为缺席、未读或在线。该模型要等一对一 Relationship Space 验证后再实施。
...
ActionPlan 过期、membership 失效、权限、商品状态或 `content_revision` 变化时安全失败。模型不能用聊天中的“已经同意”绕过 confirmation token；primary 请求重试也不能变成第二次确认。行动级 `agent_action_audits` 已在同一事务中记录提案、确认、执行和终态事件；聊天提案在共享 trace 下通过可空 `agent_run_id` 与首版 `agent_runs`/`agent_run_events` 串联，后者记录不含正文的路由、provider/model、检索聚合、工具类别、SSE TTFT、耗时和 typed outcome，并在服务端保存有界 input/output token 计数；客户端断开后的有界取消结案和跨进程 stale-run durable reconciliation 已落地。provider 侧 TTFT、设备/重新认证绑定、版本化风险文案和完整 `/api/v1` 对账仍是后续工作。
