...
第一阶段不改变服务器写模型：移动端以 `Thread + Conversation + Message` 为事实源生成空间投影。当前 API 已提供活动校园作用域的稳定 `relationship_key` 与只读 `space-events` cursor；第二阶段再增加用户 Pin 和共享对象引用。通用 Event Log 必须等双写、回放、幂等和旧客户端兼容方案完成后再成为权威写模型。
