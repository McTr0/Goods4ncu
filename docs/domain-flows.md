...
AI 只能生成资料草稿或单次回复草稿；不能生成、导入或自动发布角色皮肤，不能自动代聊或学习用户语气去维持关系。平台 Agent 与用户分身必须使用不同入口、标识和上下文提示。
...
### 目标态：从 Thread 渐进投影为 Relationship Space

同一校园内的一对用户只拥有一个长期关系入口；打开后看到的是共同空间，不是另一条无限增长的消息列表：

```text
打开关系空间
  -> 展示双方角色卡与各自主动公开的接近方式
  -> 从现有 Message / ConversationEvent / Quote 构建确定性时间轨迹
  -> 恢复最近一次连接终点、最近 Pin 和少量末尾事件
  -> 用户选择留下留言，或请求一次新的连接
...
角色随纯本地布局状态缩放是允许的，例如密集阅读时缩成角落头像；角色不得根据对方输入、打开页面、通知预览或后台活动变化。连接期间缩细 Memory Rail、弱化角色，表达“当前关注人”；连接结束后恢复历史入口。
