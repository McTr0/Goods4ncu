...
| 共同空间投影（R0） | 打开联系人线程，再打开一段留言或连接 | 双方观察页面 | 留言阶段显示双方角色锚点、时间轨迹和明确的“可以留言”；活动校园 Thread 只投影已发布 persona，列表/线程头/完整空间分别使用 24/48/160 静态 token；进入明确连接后角色 token 退到背景，只保留双方名称与“已连接”状态；草稿/归档回退普通头像。页面打开、滚动和角色缩放不产生对方可见事件。390×844 与桌面分栏都不得遮挡正文。 |
