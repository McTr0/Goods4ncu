...
| 图片审核 provider contract | `tests/moderation_worker_integration.rs::private_provider_attempt_receives_a_fresh_signed_object_url` 启动临时 HTTP provider，验证 Bearer key、稳定 `storage_key`、fresh presigned GET 和 approved 回写 | provider 超时/非 2xx/未知 verdict、队列等待导致旧签名过期、伪造 persona key | 私有任务每次领取重新签发 URL；legacy/public job 才回退持久 `image_url`；未知 verdict 不得自动通过；真实 provider 账号与样本仍需部署侧验收。 |
...
| 角色图片候选（R1） | owner 创建 `illustration/photo_stylized` 候选，请求 owner-scoped upload target，在私有部署用单对象 presigned PUT（开发部署回退 STS）后调用 `complete`；审核 worker 处理后显式选择、发布，再撤销；cleanup worker 过期孤儿 `pending_upload` | 伪造客户端 key/target、尺寸/MIME/文件头不符、未完成上传、超过 24 小时的孤儿上传、`pending_review` 期间选择、跨校园读取、非 owner 撤销、审核拒绝、远端清理失败重试 | 只接受 `persona/{campus}/{persona}/{asset}` key；PUT URL 只绑定该 key；Range/object probe 成功且大小/MIME/PNG-JPEG-WebP 文件头精确匹配后才进入 `pending_review`/`active`；未审核素材不出现在 public persona；过期 `pending_upload` 变为 `revoked`，写入 `asset_expired` 审计并进入耐久 signed DELETE；选择新素材退回 draft；撤销清除公开投影，图片 URL 失效回退静态 token，不产生在线/已读/Agent 事实。 |
