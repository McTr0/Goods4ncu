...
| 共同空间 Pin 与共享对象（R2） | 双方在一条已有消息上分别 Pin、撤销 Pin，再读取 `space-events` | 重复 Pin、撤销不存在的 Pin、隐藏源消息、跨校园读取 | Pin 幂等且可撤销；`actor_id` 保留主动者；quote 只读派生为共享对象；源消息隐藏/删除/失权后当前投影失效，不产生 read/typing/online 事实。 |
