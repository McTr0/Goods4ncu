...
| 共同空间 Pin 与共享对象（R2） | `r2-chat` 驱动与 Rust 回归共同覆盖双方 Pin、创建 file/link 权威对象、消息引用、撤销后读取 `space-events` | 重复 Pin、撤销不存在的 Pin、伪造外部文件 URL、隐藏源消息、跨校园读取、非创建者撤销 | Pin 幂等且可撤销；`actor_id` 保留主动者；file/link 只能引用活动 `chat_shared_objects`，文件 key 由服务端生成，链接片段被规范化且不抓取；撤销后原消息保留但 quote、媒体入口和共享对象投影失效，双方收到 `shared_object_revoked`；Flutter rail 只读且不自动加载资源，不产生 read/typing/online 事实。 |
